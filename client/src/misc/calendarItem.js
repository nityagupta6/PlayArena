    ...
    return (
        <div className="border-[.5px] bg-white border-gray-300 cursor-default flex justify-center items-center text-center h-[50px] w-[150px]">
            {props.type === "time" ? `${props.data < 10 ? `0${props.data}` : props.data}:00 - ${props.data + 1 < 10 ? `0${props.data + 1}` : props.data + 1}:00` :
                props.type === "courtHeader" ? <div className="h-full w-full flex justify-center items-center font-semibold bg-[#00B562]" >{`${props.data}`}</div> :
                    props.type === "timeHeader" ? <div className="h-full w-full flex justify-center items-center font-semibold bg-[#00B562]" >Slots</div> :
                        <div className="h-full w-full">

                            {props.status === "available" ?
                                <div className="relative flex justify-center items-center h-full w-full">
                                    <div onClick={handleAdd} className="hover:bg-green-200 transition-all cursor-pointer flex justify-center items-center h-full w-full">Available</div>
